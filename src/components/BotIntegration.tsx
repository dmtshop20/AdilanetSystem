...
export default function BotIntegration({
  ...
  // Config state
  const [activeProvider, setActiveProvider] = useState<"WhatsApp" | "Telegram">(
    "WhatsApp",
  );
  ...
  // Simulation states
  const [simProvider, setSimProvider] = useState<"WhatsApp" | "Telegram">(
    "WhatsApp",
  );
    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-lg font-bold text-slate-800">
          Bot WhatsApp & Telegram Portal
        </h2>
        <p className="text-xs text-slate-500">
          Membantu transaksi isi ulang & pembelian voucher hotspot secara
          otomatis 24 jam via chat messaging.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <form onSubmit={handleSaveConfig} className="space-y-4">
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`w-2.5 h-2.5 rounded-full ${apiKey ? "bg-green-500 animate-pulse" : "bg-red-400"}`}
                ></span>
                <span className="text-xs font-semibold text-slate-700">
                  {apiKey
                    ? "🟢 Terbuka & Siap Beroperasi (Simulasi Online)"
                    : "🔴 Belum Dikonfigurasi"}
                </span>
              </div>
            </div>
                ...
                <input
                  ...
                  placeholder={
                    activeProvider === "WhatsApp"
                      ? "WhatsApp Web Session ID / API Server Key"
                      : "HTTP BotFather API Token"
                  }
                  value={apiKey}
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold font-mono text-slate-400 uppercase">
                Pemberitahu Balas Otomatis (Auto-Reply)
              </label>
        ...
        <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-100 shadow-3xs overflow-hidden flex flex-col min-h-[460px]">
          ...
          <div className="p-4 border-b border-indigo-50 bg-slate-50/50 flex flex-wrap items-center justify-between gap-3 text-xs">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                value={simProvider}
                onChange={(e) =>
                  setSimProvider(e.target.value as "WhatsApp" | "Telegram")
                }
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const found = customersSafe.find(
                    (c) => c.phone === val || c.username === val,
                  );
                  if (found) {
              ...
              >
                ...
                {customersSafe.map((c) => (
                  <option
                    key={c.id}
                    value={simProvider === "WhatsApp" ? c.phone : c.username}
                  >
                    {c.name}
                  </option>
                ))}
          ...
          <div
            className={`p-4 flex-1 h-96 overflow-y-auto space-y-3 flex flex-col ${simProvider === "WhatsApp" ? "bg-[#efeae2]" : "bg-[#f4f4f7]"}`}
          >
            ...
            <div className="text-center">
              <span className="bg-white/80 backdrop-blur-xs text-[10px] text-slate-500 px-2.5 py-1 rounded-lg border border-slate-200/50 shadow-3xs inline-block font-mono">
                Chatbot Server Online. Ketik *HELP* atau *INFO* untuk mulai
                menguji.
              </span>
            </div>

            {chatLogsSafe.filter((log) => log.provider === simProvider)
              .length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-6 text-slate-400 space-y-1">
                ...
                <span className="text-xs">
                  Belum ada obrolan simulator chatbot.
                </span>
                <span className="text-[10px] text-slate-400 italic">
                  Ketik pesan di bawah dan lihat auto-reply robot.
                </span>
              </div>
            ) : (
              chatLogsSafe
                .filter((log) => log.provider === simProvider)
                .map((log, idx) => (
                  <div key={log.id} className="space-y-1.5">
                    {/* Sender user label */}
                    {log.isIncoming && (
                      <div className="text-[9px] text-slate-400 font-bold ml-1 flex items-center gap-1">
                        <User size={8} /> {log.senderName} (
                        {log.senderPhoneOrUser})
                      </div>
                    )}
                    ...
                    {log.isIncoming && (
                      <div className="flex justify-start">
                        <div className="max-w-[85%] bg-white rounded-xl p-2.5 text-xs text-slate-800 shadow-3xs border border-slate-200/30">
                          ...
                          <div className="text-[8px] text-slate-400 text-right mt-1 font-mono font-bold">
                            {log.timestamp}
                          </div>
                        </div>
                      </div>
                    ...
                    {log.replyText && (
                      <div className="flex flex-col space-y-1">
                        <div className="text-[9px] text-slate-400 font-bold mr-1 text-right flex items-center justify-end gap-1">
                          <Bot size={8} className="text-indigo-600" />{" "}
                          Auto-Robot:{" "}
                          {activeBotConfig?.botUsername || "Bot Server"}
                        </div>
                        <div className="flex justify-end">
                          <div
                            className={`max-w-[85%] rounded-xl p-2.5 text-xs shadow-3xs whitespace-pre-wrap ${
