...
import { CustomerAccount, VoucherPackage, Voucher, QrisTransaction, AppDisplayConfig } from "../types";
...
interface Props {
  ...
  onRegister: (name: string, username: string, phone: string, password?: string) => Promise<boolean>;
  onLogout: () => void;
  onBuyVoucherWithSaldo: (packageId: string) => void;
  onTriggerQrisInvoice: (amount: number, type: 'Topup' | 'DirectBuy', packageId?: string) => void;
  transactions: QrisTransaction[];
  qrislyConfig: any;
  selectedTxId: string | null;
  onSelectTx: (txId: string | null) => void;
  onSimulateWebhook: (transactionId: string, status: 'Paid' | 'Expired') => Promise<void>;
...
export default function CustomerPortal({
  ...
  const [activePortalTab, setActivePortalTab] = useState<'home' | 'buy' | 'vouchers' | 'history'>('home');
  const [connectedVoucherId, setConnectedVoucherId] = useState<string | null>(null);
  ...
  // Auto-slide effect
  useEffect(() => {
    if (!displayConfig || !displayConfig.adsImages || displayConfig.adsImages.length === 0) return;
    const intervalId = setInterval(() => {
      setCurrentSlideIndex(prev => (prev + 1) % displayConfig.adsImages.length);
    }, 4000);
  ...
  const activeTx = transactions.find(t => t.customerId === currentCustomer?.id && (t.id === selectedTxId || t.transactionId === selectedTxId));

  const autoInterval = qrislyConfig?.autoCheckInterval || 5;

  const [pollCountdown, setPollCountdown] = useState(autoInterval);
  const [isPolling, setIsPolling] = useState(false);
  const [lastCheckMessage, setLastCheckMessage] = useState<string>("Sistem standby, siap melacak.");
    ...
    const checkIntervalId = setInterval(async () => {
      try {
        ...
        if (res.ok) {
          ...
          setRawApiResponse(data.apiStatusResponse || { info: "Berjalan dalam mode simulasi", status: data.transactionStatus });
          ...
          if (data.transactionStatus === "Paid") {
            ...
            setLastCheckMessage(`Dicek pada ${new Date().toLocaleTimeString()} - Status: Belum Bayar.`);
          }
        }
      } catch (err) {
  ...
  const handleManualStatusCheck = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setRawApiResponse(data.apiStatusResponse || { status: data.transactionStatus, note: "Pilih tombol simulasi di bawah jika sandbox." });
        
        if (data.transactionStatus === "Paid") {
          alert("Selamat! Pembayaran Telah Diterima. Saldo Anda akan otomatis bertambah.");
          window.location.reload();
        } else {
          alert("Pembayaran belum terdeteksi. Silakan transfer nominal pas sesuai nominal sampai digit terakhir.");
        }
      }
    } catch (err) {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!success) {
        setLoginError("Nama Pengguna (Username), No HP, atau Kata Sandi salah. Pilih 'Daftar' jika akun baru.");
      }
    } catch (err) {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!regName || !regUser || !regPhone || !regPassword || !regConfirmPassword) {
    ...
    try {
      ...
      if (success) {
        ...
        alert("Pendaftaran berhasil! Silakan masuk menggunakan kata sandi Anda.");
      } else {
        setRegError("Username sudah terdaftar! Pilih username slang unik lain.");
      }
    } catch (err) {
  ...
  const handleConnectWifi = (voucherId: string) => {
    ...
    setTimeout(() => {
      ...
      alert("Autentikasi Router Berhasil. Anda sekarang terhubung ke Internet!");
    }, 1500);
  };
          ...
          <div className="w-full max-w-md relative aspect-[21/9] sm:aspect-[16/7] rounded-2xl overflow-hidden border border-slate-800 shadow-md mb-4 shrink-0">
            ...
            <div className="absolute bottom-2 left-0 right-0 flex justify-center gap-1.5 z-20">
              {displayConfig.adsImages.map((_: any, idx: number) => (
                <span key={idx} className={`w-1.5 h-1.5 rounded-full transition-all ${idx === currentSlideIndex ? 'bg-indigo-500 w-3' : 'bg-white/40'}`}></span>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="bg-slate-900 max-w-md w-full rounded-2xl border border-slate-850 shadow-xl overflow-hidden flex flex-col shrink-0">
          <div className="flex-1 p-5 sm:p-8 space-y-5">
            <div className="text-center space-y-2">
              ...
              <h2 className="text-lg font-extrabold text-white">Portal Member WiFi Adila.Net</h2>
              <p className="text-xs text-slate-450 text-slate-400 leading-relaxed font-medium">Beli voucher hotspot kecepatan tinggi, langsung potong saldo deposit Anda kapan saja!</p>
            </div>
            ...
            <div className="flex border-b border-slate-850 pb-0.5 text-xs">
              ...
              <button 
                type="button"
                onClick={() => { setIsRegister(true); setRegError(""); }}
                className={`flex-1 py-2 px-3 font-bold text-center border-b-2 transition-all cursor-pointer ${isRegister ? "border-indigo-500 text-indigo-400" : "border-transparent text-slate-500 hover:text-slate-400"}`}
              >
                Daftar Baru
              </button>
            ...
            {!isRegister ? (
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold font-mono text-slate-400 uppercase tracking-wider block">Username / No HP WhatsApp</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold font-mono text-slate-400 uppercase tracking-wider block">Kata Sandi Akun</label>
                  <div className="relative">
                ...
                <div className="text-[10px] text-slate-500 font-mono flex flex-col items-center justify-center gap-1 text-center bg-slate-950/40 p-2.5 rounded-xl border border-slate-900">
                  <div>Akun uji coba: <span className="font-bold text-indigo-400">andi_wijaya</span></div>
                  <div>Kata sandi default: <span className="font-bold text-indigo-400">andi_wijaya123</span></div>
                </div>
              </form>
            ) : (
              /* 2. REGISTER FORM */
              <form onSubmit={handleRegisterSubmit} className="space-y-3">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold font-mono text-slate-400 uppercase tracking-wider block">Nama Lengkap</label>
                  <input 
                    type="text" 
                    required
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold font-mono text-slate-400 uppercase tracking-wider block">Pilih Username Unik (Sandi Toko)</label>
                  <input 
                    type="text" 
                    required
                  ...
                  />
                  <p className="text-[9px] text-slate-500 italic block">Hanya huruf kecil, angka, dan underscore.</p>
                </div>

                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold font-mono text-slate-400 uppercase tracking-wider block">Nomor WhatsApp (Aktif)</label>
                  <div className="relative">
                    <Smartphone className="absolute left-3.5 top-3 text-slate-500" size={15} />
                    <input 
                      type="text" 
                      required
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold font-mono text-slate-400 uppercase tracking-wider block">Kata Sandi Akun Baru</label>
                  <div className="relative">
                    <Lock className="absolute left-3.5 top-3 text-slate-500" size={15} />
                    <input 
                      type="password" 
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold font-mono text-slate-400 uppercase tracking-wider block">Ulangi Kata Sandi Baru</label>
                  <div className="relative">
                    <Lock className="absolute left-3.5 top-3 text-slate-500" size={15} />
                    <input 
                      type="password" 
    ...
    <div className="relative pb-20 bg-slate-950 min-h-[600px] border border-slate-900 rounded-3xl overflow-hidden shadow-2xl flex flex-col font-sans max-w-md mx-auto sm:max-w-none">
      ...
      <div className="bg-slate-900 text-slate-500 text-[10px] py-1 px-4 flex justify-between items-center font-mono font-bold border-b border-slate-850">
        ...
        <div className="flex items-center gap-2">
          {connectedVoucherId ? <Wifi size={10} className="text-emerald-400" /> : <Wifi size={10} />}
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
        ...
        {connectedVoucherId && (
          <div className="bg-emerald-950/40 border border-emerald-900/50 rounded-2xl p-5 flex flex-col items-center justify-center text-center shadow-lg relative overflow-hidden animate-fade-in">
            ...
            <h3 className="text-white font-extrabold text-sm mb-1">Terhubung ke Internet</h3>
            <span className="text-xs text-slate-400 font-medium">Sisa waktu akses WiFi Anda:</span>
        ...
        {activePortalTab === 'home' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-slate-900 p-5 rounded-2xl border border-slate-850 flex flex-col items-center sm:flex-row sm:justify-between gap-4 shadow-md">
              <div className="flex flex-col items-center sm:flex-row sm:items-center gap-3 text-center sm:text-left">
                ...
                <div>
                  <h3 className="text-base font-bold text-white">Halo, {currentCustomer.name}!</h3>
                  <p className="text-[11px] text-slate-400">@{currentCustomer.username} &bull; Member Aktif</p>
                </div>
              </div>
              <button 
                onClick={onLogout}
            ...
            <div className="bg-gradient-to-br from-indigo-900/60 to-slate-900 border border-indigo-900/40 text-white p-6 rounded-2xl relative overflow-hidden shadow-xl flex flex-col justify-between gap-5">
              ...
              <div className="space-y-1 relative">
                <span className="text-[10px] font-bold font-mono tracking-widest text-indigo-300 uppercase">SALDO DEPOSIT</span>
                <h2 className="text-3xl font-black tracking-tight text-white mb-2">{formatRupiah(currentCustomer.saldo)}</h2>
                <div className="inline-flex items-center gap-1.5 text-[9px] bg-slate-950/50 text-slate-300 font-mono font-bold px-2.5 py-1 rounded-md border border-slate-800/80">
                  <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span> Saldo Aman
                </div>
              </div>
              <button 
                type="button"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div 
               ...
               >
                ...
                <span className="font-bold text-xs text-slate-200">Beli Paket</span>
              </div>
              <div 
               ...
               >
                ...
                <span className="font-bold text-xs text-slate-200">Voucher Saya</span>
              </div>
            </div>

            {/* 2. LIVE CHECKOUT IF AN ACTIVE TRANSACTION EXISTS */}
            {activeTx && (
              <div id="customer-active-checkout" className="bg-gradient-to-b from-slate-900 to-indigo-950/20 rounded-2xl border border-indigo-900/40 p-5 space-y-5 shadow-lg">
          <div className="flex items-center justify-between border-b border-indigo-950 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-white">Status Checkout QRIS: {activeTx.note}</h3>
            </div>
            <button 
              type="button"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            ...
            <div className="flex flex-col items-center justify-center p-6 bg-slate-950 rounded-2xl border border-slate-900">
              {activeTx.status === "Paid" ? (
                <div className="w-48 h-48 flex flex-col items-center justify-center text-center p-4 space-y-2">
                  ...
                  <span className="font-bold text-xs text-white">Pembayaran Sukses</span>
                  <span className="text-[10px] text-slate-500 font-mono">Saldo Anda Otomatis Bertambah</span>
                </div>
              ) : activeTx.status === "Expired" ? (
                <div className="w-48 h-48 flex flex-col items-center justify-center text-center p-4 space-y-2">
                  <AlertTriangle size={48} className="text-red-400" />
                  <span className="font-bold text-xs text-white">Tagihan Expired</span>
                  <span className="text-[10px] text-slate-500 font-mono">Silakan buat ulang</span>
                </div>
              ) : (
                <>
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                <span className="text-[9px] text-slate-500 font-mono uppercase block">NO ID INVOICE</span>
                <div className="text-sm font-extrabold text-indigo-400 font-mono tracking-wider">{activeTx.transactionId}</div>
                <div className="text-[10px] text-slate-500 font-mono">Dibuat: {activeTx.createdAt}</div>
              </div>

              <div className="p-4 bg-slate-950 rounded-xl border border-slate-900 font-mono space-y-2.5 text-xs text-slate-300">
                <div className="flex justify-between">
                  <span className="text-slate-400">Nominal Pengisian:</span>
                  <span className="font-bold text-white">{formatRupiah(activeTx.amount)}</span>
                </div>
                <div className="flex justify-between text-indigo-300 font-bold bg-indigo-950/50 px-2 py-0.5 rounded border border-indigo-900/20">
                  <span className="text-slate-400">Kode Unik Verifikasi (+):</span>
                  <span>+{activeTx.uniqueCode}</span>
                </div>
                <div className="flex justify-between border-t border-slate-900 pt-2 text-sm">
                  <span className="font-bold text-slate-450 text-slate-400">Total Wajib Transfer:</span>
                  <span className="font-extrabold text-indigo-400 underline decoration-indigo-400/20 decoration-2">{formatRupiah(activeTx.totalPayment)}</span>
                </div>
              </div>
              ...
              {activeTx.status === "Pending" && (
                <div className="p-3 bg-amber-950/30 text-amber-500 border border-amber-900/40 rounded-xl text-[10px] leading-relaxed font-semibold">
                  ⚠️ <strong>PENTING:</strong> Transfer sebesar nominal pas <strong>{formatRupiah(activeTx.totalPayment)}</strong> (jangan dibulatkan atau dikurangi) agar sistem automatically mendeteksi status lunas Anda dalam 10 detik!
                </div>
              )}
            </div>
          </div>
            ...
            <div className="bg-slate-950 text-slate-200 rounded-xl p-4 border border-slate-900 space-y-3 font-mono text-xs">
              ...
              <div className="text-[10px] space-y-1.5">
                <div className="flex justify-between text-slate-500">
                  ...
                  <span className="text-emerald-400 font-bold">Connected Realtime &bull; Sandbox</span>
                </div>
                <div className="flex justify-between text-slate-500">
                  <span>Hasil Verifikasi Gateway:</span>
                  <span className="text-amber-400 font-bold animate-pulse">{lastCheckMessage}</span>
                </div>
              </div>
              ...
              <div className="pt-2 flex flex-col sm:flex-row gap-2 font-sans">
                ...
                >
                  <RefreshCw size={11} className={manualCheckLoading ? "animate-spin" : ""} />
                ...
                {qrislyConfig?.mode !== "Production" && (
                  <button 
                    type="button"
                    onClick={async () => {
                      if (confirm("Simulasikan Pembayaran QRIS Lunas Sukses menggunakan Pembayar Sandbox?")) {
                        await onSimulateWebhook(activeTx.transactionId, "Paid");
                        alert("Simulasi lunas sandbox diproses! Saldo akun otomatis bertambah.");
                        window.location.reload();
                      }
                    }}
          ...
          {rawApiResponse && (
            <div className="bg-slate-950 rounded-xl border border-slate-900 p-3 font-mono text-[9px] text-emerald-400 overflow-x-auto max-h-[120px]">
              <span className="text-slate-500 block border-b border-slate-900 pb-1 mb-1 font-bold font-sans">DIAGNOSTICS QRISLY RESPONSE:</span>
              <pre className="whitespace-pre">{JSON.stringify(rawApiResponse, null, 2)}</pre>
            </div>
          )}
        </div>
      )}
      ...
      <div className="space-y-4 animate-fade-in">
        <div className="px-1 pt-2">
          ...
          <p className="text-xs text-slate-400">Beli voucher WiFi langsung potong saldo deposit Anda.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-4">
          {packages.map(p => {
            const canAfford = currentCustomer.saldo >= p.price;
            return (
              <div key={p.id} className="bg-slate-900 p-5 rounded-2xl border border-slate-850 shadow-md flex flex-col justify-between hover:border-indigo-900/50 transition-all">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10.5px] text-slate-400 font-mono font-bold">{p.durationHours} Jam</span>
                  </div>
                  <h4 className="font-extrabold text-white text-sm">{p.name}</h4>
                  <p className="text-[11px] text-slate-450 text-slate-400 min-h-[30px] line-clamp-2 leading-relaxed">{p.description}</p>
                </div>

                <div className="pt-4 mt-4 border-t border-slate-850 flex items-center justify-between">
                  <div>
                    <span className="text-[9px] text-slate-500 block font-mono uppercase tracking-wider">BIAYA BELI</span>
                    <span className="text-base font-black text-white">{formatRupiah(p.price)}</span>
                  </div>
                  <button 
                    onClick={() => {
                      if (!canAfford) {
                        if(confirm("Saldo Anda tidak cukup. Buat pesanan QRIS top-up saldo sekarang?")) {
                          setIsTopupModalOpen(true);
                          setTopupAmount(p.price.toString());
                        }
                        return;
                      }
                      if(confirm(`Konfirmasi pembelian ${p.name}? Sisa saldo Anda akan berkurang Rp ${p.price}`)) {
      ...
      <div className="space-y-4 animate-fade-in">
        <div className="px-1 pt-2 border-b border-slate-850 pb-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400">Total: {myVouchers.length} tiket aktif</p>
          </div>
        </div>
        ...
        {myVouchers.length === 0 ? (
          <div className="text-center py-16 px-4 bg-slate-900 border border-slate-850 rounded-2xl text-slate-500 font-medium">
            ...
            <p className="text-sm text-slate-300 font-bold mb-1">Tidak Ada Voucher</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-4">
            {myVouchers.slice().reverse().map(v => (
              <div key={v.id} className="bg-slate-900 p-5 rounded-2xl border border-slate-850 relative overflow-hidden space-y-4 shadow-md">
                ...
                <div className="space-y-1 pr-12">
                  <span className="text-[10px] text-slate-400 block font-mono font-semibold uppercase">{v.packageName}</span>
                ...
                <div className="text-[10px] text-slate-400 space-y-1.5 font-sans pb-2">
                   <div className="flex justify-between items-center bg-slate-950 p-2 rounded-lg border border-slate-850">
                    <span className="text-slate-500 font-medium font-sans">Aktivasi:</span>
                    <span className="font-mono font-bold text-indigo-300">{v.activatedAt}</span>
                  </div>
                  <div className="flex justify-between items-center bg-red-950/20 p-2 rounded-lg border border-red-950/50">
                    <span className="text-red-400/70 font-medium font-sans">Expired:</span>
                    <span className="font-mono font-bold text-red-400">{v.expiresAt}</span>
                  </div>
                </div>

                <button 
                  onClick={() => handleConnectWifi(v.id)}
                  disabled={connectedVoucherId === v.id || isConnectingWifi}
                ...
                >
                  <Wifi size={16} className={isConnectingWifi && connectedVoucherId !== v.id ? "animate-pulse" : ""} />
                  {connectedVoucherId === v.id ? "SEDANG TERHUBUNG" : isConnectingWifi ? "MENGOTENTIKASI..." : "MULAI INTERNET SEKARANG"}
                </button>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-4 animate-fade-in">
        <div className="px-1 pt-2 border-b border-slate-850 pb-3">
          ...
          <p className="text-xs text-slate-400">Tagihan topup saldo deposit.</p>
        </div>

        {transactions.filter(t => t.customerId === currentCustomer?.id).length === 0 ? (
          <div className="text-center py-16 px-4 bg-slate-900 border border-slate-850 rounded-2xl text-slate-500 font-medium">
            Mulai isi saldo menggunakan QRIS di beranda untuk melihat riwayat.
          </div>
        ) : (
          <div className="space-y-3 pb-6">
            {transactions
              ...
              .reverse()
              .map(t => (
                <div key={t.id} className="bg-slate-900 border border-slate-850 p-4 rounded-xl flex flex-col gap-3 shadow-md">
                   <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        <span className="text-[10px] text-slate-500 font-mono block">{t.createdAt}</span>
                        <div className="font-mono font-bold text-white tracking-wider text-xs">{t.transactionId}</div>
                      </div>
                      <span className={`px-2.5 py-1 text-[9px] font-bold rounded-full uppercase tracking-wider ${
                        t.status === "Paid" ? "bg-emerald-950/60 text-emerald-400 border border-emerald-900/50" :
                        t.status === "Pending" ? "bg-amber-950/60 text-amber-400 border border-amber-900/50" :
                        "bg-red-950/60 text-red-400 border border-red-900/50"
                      }`}>
                        {t.status === "Paid" ? "LUNAS" : t.status === "Pending" ? "MENUNGGU" : t.status}
                      </span>
                   </div>
                   <div className="flex items-center justify-between bg-slate-950 p-3 rounded-lg border border-slate-800">
                     <span className="text-[11px] text-slate-400">Total Wajib Transfer</span>
                     <span className="font-black font-mono text-indigo-400">{formatRupiah(t.totalPayment)}</span>
                   </div>
                   ...
                   {t.status === 'Pending' && (
                      <button 
                        ...
                        onClick={() => {
                          ...
                          setTimeout(() => {
                            document.getElementById("customer-active-checkout")?.scrollIntoView({ behavior: "smooth" });
                          }, 50);
                        }}
      ...
      <div className="absolute bottom-0 left-0 right-0 h-16 bg-slate-950/95 backdrop-blur-xl border-t border-slate-850 flex items-center justify-around px-2 z-10">
        ...
        >
          <User size={20} className={activePortalTab === 'home' ? "drop-shadow-[0_0_8px_rgba(129,140,248,0.5)]" : ""} />
        ...
        >
          <ShoppingBag size={20} className={activePortalTab === 'buy' ? "drop-shadow-[0_0_8px_rgba(129,140,248,0.5)]" : ""} />
        ...
        >
          ...
          <Ticket size={20} className={activePortalTab === 'vouchers' ? "drop-shadow-[0_0_8px_rgba(129,140,248,0.5)]" : ""} />
        ...
        >
          <Clock size={20} className={activePortalTab === 'history' ? "drop-shadow-[0_0_8px_rgba(129,140,248,0.5)]" : ""} />
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-slate-900 rounded-2xl border border-slate-850 p-6 max-w-sm w-full shadow-2xl space-y-4 text-white">
            <div className="flex items-center justify-between border-b border-slate-850 pb-3">
              ...
              <button onClick={() => setIsTopupModalOpen(false)} className="text-slate-400 hover:text-white transition duration-150 cursor-pointer border-none bg-transparent">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div className="text-xs text-slate-400 leading-relaxed font-sans">
                Layanan gateway Sanpay.id mengubah kode QRIS statis merchant Anda menjadi dynamic invoice dengan total nominal unik agar mutasi lunas langsung terdeteksi otomatis.
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-bold font-mono text-slate-500 uppercase block tracking-wider">Masukkan Jumlah Isi Ulang (Rp)</label>
                <input 
              ...
              <div className="text-[10px] text-indigo-400 bg-indigo-950/40 p-3 rounded-lg border border-indigo-900/30 leading-relaxed">
                ℹ️ Sistem akan menyisipkan kode unik (+Rp1 s.d +Rp499) secara melayang untuk deteksi instan tanpa konfirmasi manual.
              </div>
