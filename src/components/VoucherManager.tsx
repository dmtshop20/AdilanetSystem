...
export default function VoucherManager({
  ...
  const filteredVouchers = vouchersSafe.filter((v) => {
    ...
    const matchesPackage =
      packageFilter === "ALL" ? true : v.packageId === packageFilter;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-slate-800">
            Manajemen Paket & Voucher Hotspot
          </h2>
          <p className="text-xs text-slate-500">
            Buat paket kecepatan internet MikroTik dan cetak massal kode sandi
            voucher client.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {packagesSafe.length === 0 ? (
            <div className="bg-slate-50 border border-dashed border-slate-200 p-6 text-center text-xs text-slate-400 rounded-xl col-span-full">
              Belum ada paket voucher wifi. Klik "Tambah Paket" untuk membuatnya
              pertama kali!
            </div>
          ) : (
            packagesSafe.map((pkg) => {
              // Count available voucher units
              const unitCount = vouchersSafe.filter(
                (v) => v.packageId === pkg.id && v.status === "Available",
              ).length;
              return (
                <div
                  key={pkg.id}
                  className="bg-white p-5 rounded-xl border border-slate-100 shadow-3xs hover:shadow-2xs transition-all relative overflow-hidden group"
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-slate-500 min-h-[32px] line-clamp-2">
                      {pkg.description}
                    </p>
                    ...
                    <div className="pt-2 border-t border-slate-100 flex items-baseline justify-between">
                      ...
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${unitCount > 0 ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-600"}`}
                      >
                        Stok: {unitCount} pcs
                      </span>
                    </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              onClick={() => {
                if (
                  confirm(
                    "Apakah Anda yakin ingin mengosongkan semua voucher nganggur (Tersedia)?\nTindakan ini tidak memengaruhi voucher yang sudah terjual.",
                  )
                ) {
                  onClearAvailableVouchers();
                }
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
              {filteredVouchers.length === 0 ? (
                <tr>
                  ...
                  >
                    Tidak ditemukan voucher hotspot yang cocok dengan kriteria
                    filter masukan.
                  </td>
                </tr>
              ) : (
                filteredVouchers.map((v) => (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4">
                      <span
                        className={`px-2 py-0.5 font-bold text-[9px] rounded-full inline-block ${
                          v.status === "Available"
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 max-w-sm w-full shadow-lg space-y-4">
            ...
            <form onSubmit={handleTriggerGenerate} className="space-y-4">
              ...
              <p className="text-[10px] text-slate-500 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                ⚠️ Router MikroTik akan secara dinamis memetakan kode voucher
                acak ini ke profile hotspot (speed limiter) saat customer login
                pertama kali.
              </p>
