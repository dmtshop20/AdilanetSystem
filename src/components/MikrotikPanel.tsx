...
export default function MikrotikPanel({
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.realConnectionStatus?.success) {
          alert(
            "Koneksi berhasil terhubung langsung ke hardware MikroTik RouterOS!",
          );
        } else {
          alert(
            `Konfigurasi disimpan. Namun koneksi gagal dilakukan: ${data.realConnectionStatus?.error || "Tidak dapat terhubung"}`,
          );
        }
      } else {
  ...
  useEffect(() => {
    if (!configSafe.isConnected) return;
    const fetchRealUsers = async () => {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          if (data.success) {
            setRealActiveSessions(data.users);
          }
        }
      } catch (e) {
        console.warn("Failed fetching mikrotik active users");
      }
    };

    fetchRealUsers();
    const iv = setInterval(fetchRealUsers, 10000); // refresh every 10s
    return () => clearInterval(iv);
  }, [configSafe.isConnected]);

  // Combine real sessions with fallback mock if not fully connected physically but simulated
  const sessionsToRender =
    realActiveSessions && realActiveSessions.length > 0
      ? realActiveSessions.map((r) => ({
          username: r.username,
          packageName: "Active Session",
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">
            MikroTik Hotspot RouterOS API
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Integritasi router nirkabel, telemetri pemakaian resource RAM/CPU,
            serta daftar sesi pelanggan aktif.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5 space-y-4 shadow-3xs transition-colors">
          <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
            <Cpu
              size={16}
              className="text-indigo-600 dark:text-indigo-455 animate-pulse"
            />
          ...
          {configSafe.isConnected ? (
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-slate-50 dark:bg-slate-950 p-4 border border-slate-100 dark:border-slate-800 rounded-xl space-y-2">
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold block uppercase font-mono">
                    Uptime Router
                  </span>
                  <div className="text-slate-800 dark:text-slate-200 font-extrabold text-xs">
                    {uptime}
                  </div>
              ...
              <div className="border border-slate-100 dark:border-slate-800 bg-indigo-50/20 dark:bg-indigo-950/25 rounded-2xl p-4 space-y-3">
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-450 leading-relaxed">
                  Sistem otomatis melacak, mengurai, dan mendeteksi profil
                  kecepatan (Bandwidth Profil) yang dikonfigurasikan pada User
                  Profile Hotspot di MikroTik Anda:
                </p>
                <div className="flex flex-wrap gap-2 pt-1 font-mono">
                  {configSafe.detectedProfiles &&
                  configSafe.detectedProfiles.length > 0 ? (
                    configSafe.detectedProfiles.map((prof, pIdx) => (
                      <span
                        key={pIdx}
                        className="text-[10px] font-bold text-indigo-700 dark:text-indigo-300 flex items-center gap-1 bg-white dark:bg-slate-950 px-2.5 py-1 rounded-lg border border-slate-200/60 dark:border-slate-800 shadow-3xs hover:border-indigo-500 transition-colors cursor-default"
                      >
            ...
            <div className="text-center py-12 text-slate-400 dark:text-slate-500 flex flex-col items-center justify-center gap-2">
              ...
              <span>
                Harap isi kredensial dan hubungkan router untuk menampilkan data
                hardware.
              </span>
            </div>
          )}
          ...
          <form onSubmit={handleSaveSettings} className="space-y-3">
            ...
            <div className="flex flex-col gap-2 pt-2">
              ...
              >
                <Wifi size={14} />
                {isConnecting
                  ? "Mencoba Koneksi Router..."
                  : configSafe.isConnected
                    ? "Uji Koneksi Router Ulang"
                    : "Koneksikan & Uji Router"}
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="p-4 bg-slate-50/50 dark:bg-slate-950/40 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-bold font-mono uppercase tracking-wider text-slate-500 dark:text-slate-400">
              Live Active Hotspot Sessions ({sessionsToRender.length})
            </h4>
          </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-[11px] font-semibold text-slate-600 dark:text-slate-400">
              {!configSafe.isConnected ? (
                <tr>
                  <td
                    colSpan={7}
                    className="p-8 text-center text-amber-600 bg-amber-50/20 dark:bg-amber-950/10"
                  >
                    ⚠️ MikroTik terputus. Silakan hubungkan router di atas untuk
                    menarik data sesi live.
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  >
                    Belum ada voucher hotspot yang login aktif ke router
                    MikroTik.
                  </td>
                </tr>
              ) : (
                sessionsToRender.map((ses, i) => (
                  <tr
                    key={i}
                    className="hover:bg-slate-50/50 dark:hover:bg-slate-950/40 transition-colors"
                  >
                    <td className="p-3 px-4 font-bold text-slate-800 dark:text-slate-200 font-mono bg-slate-50/40 dark:bg-slate-950/20">
                      {ses.username}
                    </td>
                    ...
                    <td className="p-3 px-4 text-emerald-600 font-mono">
                      {ses.rxRate}
                    </td>
                    <td className="p-3 px-4 text-indigo-600 font-mono">
                      {ses.txRate}
                    </td>
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            if (data.success) {
                              alert(
                                `Hotspot User ${ses.username} berhasil di-kick dari Router.`,
                              );
                            } else {
                              alert(
                                `Gagal memutuskan sesi ${ses.username} di Router.`,
                              );
                            }
                          } catch (err: any) {
                            alert(
