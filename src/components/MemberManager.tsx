...
interface Props {
  ...
  onAddCustomer: (name: string, username: string, phone: string, password?: string) => void;
...
export default function MemberManager({
  ...
  const handleCreateCustomer = (e: React.FormEvent) => {
    ...
    onAddCustomer(custName, custUsername.toLowerCase().trim(), custPhone, custPassword);
    setCustName("");
    setCustUsername("");
    setCustPhone("");
    setCustPassword("");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-slate-800">Manajemen Pelanggan & Deposit Saldo</h2>
          <p className="text-xs text-slate-500">Daftarkan akun portal pelanggan khusus dan isi saldo manual bagi yang menyetor tunai.</p>
        </div>
      ...
      <div className="bg-white border border-slate-100 rounded-2xl shadow-3xs overflow-hidden">
        <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <span className="text-[11px] text-slate-400 font-mono font-bold">Total Terdaftar: {customers.length} Orang</span>
        </div>
              ...
              {filteredCustomers.length === 0 ? (
                <tr>
                  <td colSpan={6} className="text-center py-12 text-slate-400 text-xs">
                    Belum ada pelanggan terdaftar yang cocok dengan pencarian kata kunci.
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={c.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="py-3.5 px-4">
                      <div>
                        ...
                        <div className="text-[10px] text-indigo-600 font-mono font-bold">@{c.username}</div>
                      </div>
                    </td>
                    <td className="py-3.5 px-4 font-mono text-slate-500">
                      <div>{c.phone}</div>
                      <div className="text-[9px] text-slate-400 font-sans">{c.joinedDate}</div>
                    </td>
                    <td className="py-3.5 px-4">
                      <span className={`px-2 py-0.5 text-[9px] font-bold rounded-full ${
                        c.registeredVia === "WebPortal" ? "bg-indigo-50 text-indigo-700" : "bg-slate-100 text-slate-600"
                      }`}>
                        {c.registeredVia === "WebPortal" ? "WEB PORTAL" : "REGISTER BY ADM"}
                      </span>
                    </td>
                    ...
                    <td className="py-3.5 px-4">
                      <span className={`px-2.5 py-0.5 text-[9px] font-bold rounded-full border ${
                        c.status === "Active" ? "bg-emerald-50 text-emerald-700 border-emerald-100" : "bg-red-50 text-red-600 border-red-100"
                      }`}>
                    ...
                    <td className="py-3.5 px-4">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button 
                          ...
                          title={c.status === "Active" ? "Tangguhkan Pelanggan" : "Aktifkan Pelanggan"}
                        >
                        ...
                        <button 
                          onClick={() => {
                            if (confirm(`Apakah Anda yakin ingin menghapus pelanggan @${c.username}?\nTindakan ini ireversibel.`)) {
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 max-w-sm w-full shadow-lg space-y-4">
            ...
            <form onSubmit={handleCreateCustomer} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold font-mono text-slate-400 uppercase">Nama Lengkap</label>
                <input 
                  type="text" required
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold font-mono text-slate-400 uppercase">Username Portal</label>
                <input 
                  type="text" required
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold font-mono text-slate-400 uppercase">No WhatsApp Aktif</label>
                <input 
                  type="text" required
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold font-mono text-slate-400 uppercase">Kata Sandi Akun</label>
                <input 
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 max-w-sm w-full shadow-lg space-y-4">
            ...
            <form onSubmit={handleManualTopupSubmit} className="space-y-4">
              <div className="space-y-1 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                <span className="text-[10px] text-slate-400 font-bold block uppercase font-mono">Penerima Sandi</span>
                <span className="text-xs font-extrabold text-slate-800">{selectedCustObj?.name} (@{selectedCustObj?.username})</span>
                <span className="text-[10px] text-slate-500 block">Saldo Terkini: {selectedCustObj ? formatRupiah(selectedCustObj.saldo) : "Rp0"}</span>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold font-mono text-slate-400 uppercase">Jumlah Saldo yang Dititipkan (Rp)</label>
                <input 
                  type="number" n-min="500" n-max="1000000"
                  required
