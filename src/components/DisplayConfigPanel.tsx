...
export default function DisplayConfigPanel({
  ...
  const [adminPassword, setAdminPassword] = useState(
    displayConfig?.adminPassword || "admin",
  );

  const handleUpdate = () => {
    onUpdateDisplayConfig(
      runningText,
      adsImages.filter((i) => i.trim() !== ""),
      adminPassword,
    );
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Atur Running Text (Marquee) dan Foto Banner yang akan tampil di
            Portal Pelanggan agar aplikasi terlihat lebih modern dan interaktif
            seperti Android Native.
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Silakan ubah sandi bawaan lama untuk mengamankan akses pengaturan
            router Anda.
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Masukkan link URL gambar rasio 21:9 atau 16:9 agar fit di perangkat.
          </p>
