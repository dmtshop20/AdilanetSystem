...
export default function QrGateway({
  ...
  const [sanpayMode, setSanpayMode] = useState<'Simulation' | 'Production'>("Simulation");
  const [showSanpaySecret, setShowSanpaySecret] = useState(false);
  const [showKey, setShowKey] = useState(false);

  // Gemini AI Studio Secrets info
  const [envSecretsActive, setEnvSecretsActive] = useState<{ apiKey: boolean; merchantId: boolean; secretKey: boolean }>({
    apiKey: false,
    merchantId: false,
    secretKey: false
  });
  ...
  const activeTx = transactions.find(t => t.id === selectedTxId || t.transactionId === selectedTxId);
  ...
  useEffect(() => {
    ...
    const checkIntervalId = setInterval(async () => {
      try {
        setIsPolling(true);
        setLastCheckMessage(`Menghubungi endpoint verifikasi untuk ${activeTx.transactionId}...`);
  ...
  const handleManualStatusCheck = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setRawApiResponse(data.apiStatusResponse || { status: data.transactionStatus, note: "Server verifikator sandbox memberi sinyal standby." });
        
        if (data.transactionStatus === "Paid") {
          alert("Selamat! Pembayaran Telah Diterima. Paket Voucher Wifi otomatis aktif.");
          window.location.reload();
        } else {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-indigo-950 text-white rounded-2xl p-6 shadow-sm border border-slate-800 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <span className={`text-[10px] font-bold px-3 py-1.5 rounded-xl border flex items-center gap-1.5 font-mono ${
              sanpayMode === "Production" 
                ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/30" 
                : "bg-amber-500/15 text-amber-400 border-amber-500/30"
            }`}>
              <span className={`w-2 h-2 rounded-full ${
                sanpayMode === "Production" ? "bg-emerald-500 animate-pulse" : "bg-amber-500 animate-pulse"
              }`}></span>
              {sanpayMode === "Production" ? "LIVE PRODUCTION API" : "SIMULATION SANDBOX"}
            </span>
            ...
            >
              <Settings size={14} className={isConfigOpen ? "animate-spin" : ""} />
              Setelan Gateway
            </button>
          </div>
        </div>
        ...
        {isConfigOpen && (
          <form onSubmit={saveConfiguration} className="bg-slate-950/60 p-5 rounded-xl border border-slate-800 space-y-5 text-xs text-left">
            <div className="space-y-4">
              ...
              {Object.values(envSecretsActive).some(Boolean) && (
                <div className="bg-indigo-950/40 p-3 rounded-lg border border-indigo-800/40 text-[11px] text-indigo-200 font-sans leading-relaxed">
                  💡 <strong>Informasi Gateway:</strong> Beberapa kredensial Sanpay terdeteksi diisi langsung melalui <strong>Secrets panel di Gemini AI Studio</strong> Anda. Nilai tersebut otomatis diutamakan. Anda tetap bisa menimpa nilainya dengan form di bawah ini.
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-bold font-mono text-slate-350 uppercase">Sanpay API Key (Bearer Token)</label>
                    {envSecretsActive.apiKey && <span className="text-[9px] text-emerald-400 font-mono font-bold">✓ Diisi dari Secret</span>}
                  </div>
                  <div className="relative">
                    <input 
                      type={showKey ? "text" : "password"}
                      placeholder={envSecretsActive.apiKey ? "•••••••••••••••••••• (Terisi Otomatis)" : "Masukkan API Key Sanpay Anda"}
                      value={sanpayApiKey}
                  ...
                  <p className="text-[9px] text-slate-500 font-mono">Dapatkan di Dashboard Sanpay &rarr; Pengaturan API</p>
                </div>

                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-bold font-mono text-slate-350 uppercase">Sanpay Merchant ID</label>
                    {envSecretsActive.merchantId && <span className="text-[9px] text-emerald-400 font-mono font-bold">✓ Diisi dari Secret</span>}
                  </div>
                  <input 
                    type="text" 
                    placeholder={envSecretsActive.merchantId ? "(Terisi Otomatis)" : "Contoh: S1930"}
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-bold font-mono text-slate-350 uppercase">Sanpay Secret / Signature Key</label>
                    {envSecretsActive.secretKey && <span className="text-[9px] text-emerald-400 font-mono font-bold">✓ Diisi dari Secret</span>}
                  </div>
                  <div className="relative">
                    <input 
                      type={showSanpaySecret ? "text" : "password"}
                      placeholder={envSecretsActive.secretKey ? "•••••••••••••••••••• (Terisi Otomatis)" : "Masukkan Sanpay Secret Key"}
                    ...
                    <button 
                      ...
                      className="absolute right-3 top-2.5 text-slate-400 hover:text-slate-200"
                    >
                      {showSanpaySecret ? <EyeOff size={14} /> : <Eye size={14} />}
                    </button>
                  </div>
                </div>

                <div className="space-y-1">
                  <label className="text-[10px] font-bold font-mono text-slate-350 uppercase">Mode Pelacak Sanpay</label>
                  <select 
                    value={sanpayMode}
                  ...
                  >
                    <option value="Simulation">Simulation / Sandbox Mode (Simulasi Sehat)</option>
                    <option value="Production">Live Production (Hubungkan Sanpay Real API)</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-900 p-4 rounded-xl border border-indigo-500/35 border-dashed space-y-3">
              ...
              <p className="text-[10px] text-slate-300 leading-relaxed font-sans font-medium">
                Sistem secara otomatis menyediakan URL Webhook Callback untuk menyambungkan status pembayaran instan dari server Sanpay ke Router / Bot WiFi Anda:
              </p>
              <div className="flex items-center gap-2 bg-black/60 p-2.5 rounded-lg border border-slate-800">
                ...
                <button
                  type="button"
                  onClick={() => {
                    navigator.clipboard.writeText(`${window.location.origin}/api/qris/webhook`);
            ...
            <div className="pt-2 border-t border-slate-800 flex items-center justify-between">
              <span className="text-[10px] text-indigo-400 font-mono flex items-center gap-1">
                <Info size={11} /> Nilai API key disimpan aman server-side demi menjaga keamanan.
              </span>
      ...
      {activeTx ? (
        <div className="max-w-2xl mx-auto bg-white rounded-2xl border border-slate-100 shadow-md overflow-hidden">
          ...
          <div className={`p-4 text-center text-xs font-bold font-mono tracking-wider text-white ${
            activeTx.status === "Paid" ? "bg-emerald-600" :
            activeTx.status === "Pending" ? "bg-amber-500 animate-pulse" : "bg-red-600"
          }`}>
            {activeTx.status === "Paid" ? "✅ QRIS DI-VERIFIKASI LUNAS" :
             activeTx.status === "Pending" ? "⏳ MENUNGGU SCAN PEMBAYARAN" : "❌ INVOICE KEDALUWARSA/EXPIRED"}
          </div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
              ...
              <div className="flex flex-col items-center justify-center p-4 bg-slate-50 rounded-2xl border border-slate-200/60 relative">
                {activeTx.status === "Paid" ? (
                  <div className="w-48 h-48 bg-white border border-slate-200 rounded-xl flex flex-col items-center justify-center text-center p-4 space-y-2">
                    ...
                    <span className="font-bold text-xs text-slate-800">Pembayaran Sukses</span>
                    <span className="text-[10px] text-slate-400 font-mono">Diterima via QRISLY</span>
                  </div>
                ) : activeTx.status === "Expired" ? (
                  <div className="w-48 h-48 bg-white border border-slate-200 rounded-xl flex flex-col items-center justify-center text-center p-4 space-y-2">
                    <AlertTriangle size={48} className="text-red-500" />
                    <span className="font-bold text-xs text-slate-800">Invoice Expired</span>
                    <span className="text-[10px] text-slate-400 font-mono">Waktu bayar habis</span>
                  </div>
                ) : (
                  <>
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  <span className="text-[10px] text-slate-400 font-mono uppercase font-bold block">Penerima Tagihan</span>
                  <div className="text-sm font-bold text-slate-800">{activeTx.customerName}</div>
                  <div className="text-[10px] font-mono text-slate-505 text-indigo-600 font-bold">ID Transaksi: {activeTx.transactionId}</div>
                </div>
                ...
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/60 font-mono space-y-1.5 text-xs text-slate-700">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-slate-900">{formatRupiah(activeTx.amount)}</span>
                  </div>
                  ...
                  <div className="flex justify-between border-t border-slate-200 pt-1.5 text-sm">
                    <span className="font-bold text-slate-900">Total Transfer:</span>
                    <span className="font-extrabold text-indigo-600">{formatRupiah(activeTx.totalPayment)}</span>
                  </div>
                </div>
              ...
              <div className="bg-slate-900 text-slate-200 rounded-xl p-4 border border-slate-800 space-y-3 font-mono">
                ...
                <div className="text-[10px] space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-slate-300">GET /api/qris/check-status/{activeTx.transactionId}</span>
                  </div>
                  <div className="flex items-center justify-between">
                ...
                <div className="pt-2 flex gap-2">
                  ...
                  >
                    <RefreshCw size={12} className={manualCheckLoading ? "animate-spin" : ""} />
            ...
            {rawApiResponse && (
              <div className="bg-slate-950 rounded-xl border border-slate-800 p-3 space-y-1.5 font-mono text-[9px] text-emerald-400">
                <span className="text-slate-400 block border-b border-slate-800 pb-1 text-[10px] font-bold">RAW RESPONSE DIAGNOSTIK RAJAONGKIR API:</span>
            ...
            {activeTx.status === "Pending" && (
              <div className="bg-amber-50 border border-amber-200/60 p-4 rounded-xl space-y-3">
                ...
                <p className="text-[11px] text-amber-950/80 leading-relaxed font-medium">
                  Jika sedang menguji alur tanpa menyambungkan rekening bank asli, Anda dapat mengirim payload pembayaran sukses lunas di simulator berikut:
                </p>
                <div className="flex items-center gap-2">
                  <button 
                    type="button"
                    onClick={async () => {
                      if(confirm("Simulasikan pembayaran lunas sukses via Webhook?")) {
          ...
          <div className="space-y-6">
            <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-3xs space-y-4">
              ...
              <form onSubmit={(e) => {
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold font-mono text-slate-400 uppercase">Nama Penyumbang / Member</label>
                  <input 
                    type="text" 
                    required
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold font-mono text-slate-400 uppercase">Nominal Pembayaran (Rp)</label>
                  <input 
                  ...
                  {filteredTxs.length === 0 ? (
                    ...
                    filteredTxs.map(tx => (
                      <tr key={tx.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="py-3 px-4 font-mono font-bold text-slate-900">{tx.transactionId}</td>
                        <td className="py-3 px-4">{tx.customerName}</td>
                        <td className="py-3 px-4 font-mono font-bold">{formatRupiah(tx.amount)}</td>
                        <td className="py-3 px-4 font-mono text-indigo-600 font-semibold">+{tx.uniqueCode}</td>
                        <td className="py-3 px-4 font-mono font-extrabold text-slate-800">{formatRupiah(tx.totalPayment)}</td>
                        <td className="py-3 px-4">
                          <span className={`px-2 py-0.5 text-[9px] font-bold rounded-full ${
                            ...
                            {tx.status === "Paid" ? "Selesai" : tx.status === "Pending" ? "Pending" : tx.status}
                          </span>
                        </td>
